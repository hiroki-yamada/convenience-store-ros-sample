...
CleanupFetchTeleopKey::CleanupFetchTeleopKey()
{
...
void CleanupFetchTeleopKey::rosSigintHandler(int sig)
{
  ros::shutdown();
}


int CleanupFetchTeleopKey::canReceive( int fd )
{
  fd_set fdset;
  int ret;
...
void CleanupFetchTeleopKey::jointStateCallback(const sensor_msgs::JointState::ConstPtr& joint_state)
{
...
void CleanupFetchTeleopKey::moveBaseTwist(double linear_x, double linear_y, double angular_z)
{
...
void CleanupFetchTeleopKey::operateTorso(const double torso_lift_pos, const double duration_sec)
{
...
void CleanupFetchTeleopKey::operateHead(const double head_1_pos, const double head_2_pos, const double duration_sec)
{
  trajectory_msgs::JointTrajectory joint_trajectory;
...
void CleanupFetchTeleopKey::operateHead(const std::string &joint_name, const double add_pos, const double duration_sec)
{
...
void CleanupFetchTeleopKey::operateArm(const std::vector<double> & positions, const double duration_sec)
{
  trajectory_msgs::JointTrajectory joint_trajectory;
...
void CleanupFetchTeleopKey::operateArm(const int joint_number, const double add_pos, const double duration_sec)
{
...
//double CleanupFetchTeleopKey::getDurationRot(const double next_pos, const double current_pos)
//{
//  return std::max<double>((std::abs(next_pos - current_pos) * 1.2), 1.0);
//}

void CleanupFetchTeleopKey::operateHand(bool is_hand_open)
{
  trajectory_msgs::JointTrajectory joint_trajectory;
...
int CleanupFetchTeleopKey::run()
{
  ...
  sub_joint_state_        = node_handle_.subscribe<sensor_msgs::JointState>         ("/joint_states", 10, &CleanupFetchTeleopKey::jointStateCallback, this);
...
//        }
        case KEYCODE_B:
        {
          ...
          operateArm({ 0.0, 1.0, 0.0, -2.25, 0.0, +1.75, 0.0 }, 3.0);
          break;
        }
//        case KEYCODE_C:
//        {
...
{
  ros::init(argc, argv, "cleanup_fetch_teleop_key");
  CleanupFetchTeleopKey teleop_key;
